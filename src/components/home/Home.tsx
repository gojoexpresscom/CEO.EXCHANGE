...
export default function Home({ onLogout }: { onLogout?: () => void }) {
  ...
  const loadMarkets = useCallback(async () => {
    const { data, error: e } = await supabase.from("market_tickers").select("symbol,last_price,change_24h,volume_24h,updated_at").order("volume_24h", { ascending: false, nullsFirst: false }).limit(100);
    if (!e) setMarkets((data ?? []) as Market[]);
  }, []);
  ...
  const filteredMarkets = useMemo(() => {
    let list = [...markets];
    if (marketTab === "Gainers") list.sort((a, b) => Number(b.change_24h ?? 0) - Number(a.change_24h ?? 0));
    else if (marketTab === "Losers") list.sort((a, b) => Number(a.change_24h ?? 0) - Number(b.change_24h ?? 0));
    else if (marketTab === "New") list.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
    else list.sort((a, b) => Number(b.volume_24h ?? 0) - Number(a.volume_24h ?? 0));
    ...
    if (marketSearch.trim()) list = list.filter((m) => m.symbol.toLowerCase().includes(marketSearch.trim().toLowerCase()));
    return list.slice(0, 12);
  }, [marketSearch, marketTab, markets]);
  ...
  const createDeposit = async (asset: string, network: string, amount: string) => {
    ...
    notify("Deposit invoice created.");
  };

  const requestWithdrawalOtp = async () => {
    const { data, error: e } = await supabase.functions.invoke("send-otp", { body: { purpose: "withdrawal" } });
    if (e) return notify(e.message);
    if (data?.email_delivered === false) notify("Withdrawal OTP was generated but email delivery is not configured yet. Add BREVO_API_KEY to the send-otp function before using withdrawals.");
    ...
    <div style={styles.page}>
      <header style={styles.header}>
        ...
        <div style={styles.topSearch}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search" aria-label="Search CEO Exchange" />
      ...
      <main style={styles.content}>
        <section style={styles.balanceCard}>
          ...
          <div style={styles.walletVisual}>
            <div style={{ ...styles.coin, ...styles.coinGold }}>₿</div>
            <div style={styles.walletShape}><div style={styles.walletMark}>CEO</div></div>
            <div style={{ ...styles.coin, ...styles.coinSilver }}>Ξ</div>
          </div>
        </section>
        ...
        <section style={styles.marketSection}>
          <div style={styles.marketSearch}>
            ...
            <input value={marketSearch} onChange={(e) => setMarketSearch(e.target.value)} placeholder="Search markets…" />
          </div>
          ...
          <div style={styles.marketHeader}><span>Pair</span><span>Last Price</span><span>24h Change</span><span>Action</span></div>
        ...
        <section style={styles.feedSection}>
          ...
          {(feedTab === "CEO Exchange" || feedTab === "Following") && <>
            {admin && <button style={styles.createPostButton} onClick={() => setModal("post")}><Icon name="plus" size={19} />Create post</button>}
      ...
      {modal === "deposit" && <DepositModal networks={networks} deposits={deposits} onClose={closeModal} onDeposit={createDeposit} />}
...
function MarketRow({ market, favorite, onFavorite, onTrade }: { market: Market; favorite: boolean; onFavorite: () => void; onTrade: () => void }) {
  const change = Number(market.change_24h ?? 0);
  return <div style={styles.marketRow}><button style={styles.starButton} onClick={onFavorite}><Icon name="star" size={19} /></button><div style={styles.pair}><b>{market.symbol}</b><span>Vol {market.volume_24h == null ? "—" : formatMoney(Number(market.volume_24h))}</span></div><div style={styles.price}>{formatPrice(Number(market.last_price))}</div><div style={{ ...styles.change, color: change >= 0 ? "#22c7a4" : "#ff6574" }}>{change >= 0 ? "+" : ""}{change.toFixed(2)}%</div><button style={styles.tradeButton} onClick={onTrade}>Trade</button></div>;
}
...
function DepositModal({ networks, deposits, onClose, onDeposit }: { networks: Network[]; deposits: Deposit[]; onClose: () => void; onDeposit: (asset: string, network: string, amount: string) => Promise<void> }) {
  const [asset, setAsset] = useState(networks[0]?.assets?.symbol || "USDT");
  const assetNetworks = networks.filter((n) => (n.assets?.symbol || "").toUpperCase() === asset.toUpperCase());
  const [network, setNetwork] = useState(assetNetworks[0]?.network_name || networks[0]?.network_name || "");
  const [amount, setAmount] = useState("");
  useEffect(() => { const next = networks.filter((n) => (n.assets?.symbol || "").toUpperCase() === asset.toUpperCase()); setNetwork(next[0]?.network_name || ""); }, [asset, networks]);
  return <ModalShell title="Deposit" onClose={onClose}><p style={styles.modalHint}>Create a real NOWPayments deposit invoice. Your wallet balance changes only after the server confirms the payment.</p><label style={styles.label}>Asset<select style={styles.input} value={asset} onChange={(e) => setAsset(e.target.value)}>{[...new Set(networks.map((n) => n.assets?.symbol).filter(Boolean))].map((x) => <option key={x}>{x}</option>)}</select></label><label style={styles.label}>Network<select style={styles.input} value={network} onChange={(e) => setNetwork(e.target.value)}>{assetNetworks.map((n) => <option key={n.id}>{n.network_name}</option>)}</select></label><label style={styles.label}>Amount<input style={styles.input} type="number" min="0" step="any" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Amount" /></label><button style={styles.primaryButtonFull} onClick={() => void onDeposit(asset, network, amount)}><Icon name="download" size={19} />Create Deposit Invoice</button><div style={styles.divider} /><h3 style={styles.smallTitle}>Recent deposits</h3>{deposits.slice(0, 8).map((d) => <div key={d.id} style={styles.listRow}><span><b>{d.asset}</b> {d.amount}</span><span style={styles.status}>{d.status || "PENDING"}</span></div>)}{!deposits.length && <Empty text="No deposits yet." />}</ModalShell>;
}
...
const styles: Record<string, React.CSSProperties> = {
  page: { minHeight: "100vh", background: BG, color: "#fff", paddingBottom: 92, fontFamily: "Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif" },
  header: { position: "sticky", top: 0, zIndex: 20, minHeight: 86, padding: "14px 16px", display: "grid", gridTemplateColumns: "auto minmax(160px,1fr) auto", gap: 14, alignItems: "center", background: "rgba(5,5,5,.96)", backdropFilter: "blur(14px)", borderBottom: `1px solid ${BORDER}` },
  brand: { display: "flex", alignItems: "center", gap: 9, minWidth: 0 },
  logo: { width: 46, height: 46, objectFit: "contain", borderRadius: 12 },
  brandName: { maxWidth: 100, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 700, fontSize: 15 },
  topSearch: { height: 46, display: "flex", alignItems: "center", gap: 8, padding: "0 13px", border: `1px solid ${BORDER}`, borderRadius: 14, background: "#0b0b0b", color: "#777" },
  "topSearch input": { flex: 1, minWidth: 0, border: 0, outline: 0, background: "transparent", color: "#fff", fontSize: 15 },
  headerActions: { display: "flex", gap: 7 },
  iconSquare: { position: "relative", width: 46, height: 46, border: `1px solid #211b0d`, borderRadius: 14, background: "#090909", color: GOLD_LIGHT, display: "grid", placeItems: "center", cursor: "pointer" },
  ...
  content: { width: "min(760px,100%)", margin: "0 auto", padding: "14px 16px 28px" },
  balanceCard: { minHeight: 286, borderRadius: 26, border: `1px solid #1f1a0e`, background: "radial-gradient(circle at 88% 22%,#4a3512 0,#1a150b 20%,#0e0e0e 48%,#0b0b0b 100%)", padding: "28px 24px", display: "flex", justifyContent: "space-between", overflow: "hidden", position: "relative" },
  balanceInfo: { position: "relative", zIndex: 2, maxWidth: 390 },
  muted: { color: "#aaa", fontSize: 17, marginBottom: 5 },
  balanceLine: { display: "flex", alignItems: "center", gap: 9 },
  "balanceLine strong": { fontSize: 45, letterSpacing: -1.4 },
  eyeButton: { border: 0, background: "transparent", color: "#aaa", cursor: "pointer", padding: 4 },
  subtle: { color: "#777", fontSize: 13, marginTop: 8 },
  balanceButtons: { display: "flex", gap: 12, marginTop: 30, flexWrap: "wrap" },
  primaryButton: { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, border: 0, borderRadius: 13, padding: "12px 20px", background: `linear-gradient(135deg,${GOLD},#d98e00)`, color: "#090909", fontWeight: 800, cursor: "pointer" },
  secondaryButton: { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, border: `1px solid #8b6416`, borderRadius: 13, padding: "11px 20px", background: "transparent", color: GOLD_LIGHT, fontWeight: 700, cursor: "pointer" },
  walletVisual: { width: 250, position: "relative", opacity: .95, display: "grid", placeItems: "center" },
  walletShape: { width: 185, height: 130, borderRadius: "14px 14px 24px 24px", background: "linear-gradient(145deg,#171717,#030303)", border: "2px solid #6d4c10", transform: "rotate(-8deg)", boxShadow: "0 24px 40px rgba(0,0,0,.7),inset 0 0 0 2px #18110a", display: "grid", placeItems: "center" },
  walletMark: { width: 58, height: 58, borderRadius: 18, border: `2px solid ${GOLD}`, color: GOLD, display: "grid", placeItems: "center", fontWeight: 900, fontSize: 11 },
  coin: { position: "absolute", width: 64, height: 64, borderRadius: "50%", display: "grid", placeItems: "center", fontSize: 31, fontWeight: 800, border: "2px solid #d99b19", background: "radial-gradient(circle at 30% 25%,#ffe08a,#c58409 70%,#5d3900)", color: "#201400", boxShadow: "0 10px 20px #000" },
  coinGold: { top: 4, right: 25 },
  coinSilver: { bottom: 20, left: 0, width: 56, height: 56, background: "radial-gradient(circle at 30% 25%,#ddd,#666)", borderColor: "#777", color: "#202020" },
  quickGrid: { display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, margin: "24px 8px 28px" },
  quickAction: { border: 0, background: "transparent", color: "#fff", display: "flex", flexDirection: "column", alignItems: "center", gap: 9, fontSize: 14, cursor: "pointer" },
  quickIcon: { width: 70, height: 70, borderRadius: 18, background: "#0c0c0c", border: "1px solid #151515", color: GOLD_LIGHT, display: "grid", placeItems: "center", boxShadow: "0 8px 22px rgba(0,0,0,.3)" },
  marketSection: { marginTop: 6 },
  marketSearch: { height: 58, display: "flex", alignItems: "center", gap: 11, padding: "0 17px", border: `1px solid #735316`, borderRadius: 17, background: "#090909", color: "#777", marginBottom: 17 },
  "marketSearch input": { flex: 1, border: 0, outline: 0, background: "transparent", color: "#fff", fontSize: 17 },
  tabsRow: { display: "flex", overflowX: "auto", gap: 8, borderBottom: "1px solid #242424", scrollbarWidth: "none" },
  tab: { whiteSpace: "nowrap", border: 0, background: "transparent", color: "#888", padding: "13px 10px", display: "inline-flex", gap: 6, alignItems: "center", cursor: "pointer", fontWeight: 600 },
  tabActive: { color: GOLD_LIGHT, borderBottom: `2px solid ${GOLD}` },
  marketHeader: { display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr .7fr", gap: 8, padding: "14px 8px 9px", color: "#777", fontSize: 12 },
  marketRow: { minHeight: 72, display: "grid", gridTemplateColumns: "32px 1.4fr 1fr 1fr .7fr", alignItems: "center", gap: 7, padding: "10px 8px", borderRadius: 13, background: "#101010", marginBottom: 7 },
  starButton: { border: 0, background: "transparent", color: GOLD_LIGHT, padding: 0, cursor: "pointer" },
  pair: { display: "flex", flexDirection: "column", minWidth: 0 },
  ...
  tradeButton: { border: `1px solid #785716`, borderRadius: 11, background: "transparent", color: GOLD_LIGHT, padding: "9px 8px", cursor: "pointer", fontWeight: 700 },
  ...
  modal: { width: "min(680px,100%)", maxHeight: "92vh", overflowY: "auto", background: "#0b0b0b", border: "1px solid #2a2110", borderRadius: "24px 24px 0 0", padding: 18, boxShadow: "0 -20px 60px rgba(0,0,0,.6)" },
  ...
  "modalHeader h2": { margin: 0, fontSize: 21 },
  ...
  input: { width: "100%", minHeight: 46, border: "1px solid #302711", borderRadius: 12, outline: 0, background: "#101010", color: "#fff", padding: "10px 12px" },
  textarea: { width: "100%", minHeight: 120, resize: "vertical", border: "1px solid #302711", borderRadius: 12, outline: 0, background: "#101010", color: "#fff", padding: "10px 12px", lineHeight: 1.5 },
