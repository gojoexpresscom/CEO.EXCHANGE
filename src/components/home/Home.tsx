...
function DepositModal({ networks, deposits, result, onClose, onDeposit }: { networks: Network[]; deposits: Deposit[]; result: any; onClose: () => void; onDeposit: (asset: string, network: string, amount: string) => Promise<any> }) {
  const enabledNetworks = networks.filter((n) => n.is_active !== false && (n.deposit_enabled === true || n.payout_provider === "NOWPAYMENTS"));
  const assets = [...new Map(enabledNetworks.map((n) => [String(n.assets?.symbol ?? "").toUpperCase(), n.assets])).values()].filter(Boolean) as { symbol: string; name: string }[];
  const [asset, setAsset] = useState(assets[0]?.symbol || "USDT");
  const assetNetworks = enabledNetworks.filter((n) => (n.assets?.symbol || "").toUpperCase() === asset.toUpperCase());
  const [network, setNetwork] = useState(assetNetworks[0]?.network_name || "");
  const [amount, setAmount] = useState("");

  useEffect(() => {
    const next = enabledNetworks.filter((n) => (n.assets?.symbol || "").toUpperCase() === asset.toUpperCase());
  ...
  return (
    <ModalShell title="Deposit" onClose={onClose}>
      <div style={styles.infoBox}><Icon name="shield" size={18} /><span>Deposits are created through the live CEO Exchange payment rail. Your balance is credited only after the server receives and verifies the provider confirmation.</span></div>
      {!enabledNetworks.length ? (
        <div style={styles.emptyPanel}><b>Crypto deposits are not enabled yet.</b><p>There is no active deposit network configured in Supabase. This page does not create fake addresses.</p></div>
      ) : (
        <>
          ...
          <button style={styles.primaryButtonFull} disabled={!network} onClick={() => void onDeposit(asset, network, amount)}><Icon name="download" size={19} />Generate Deposit Address</button>

          {result?.pay_address && (
            <div style={styles.depositResult}>
              <div style={styles.resultTitle}>Deposit address</div>
              ...
              {result.invoice_url && <button type="button" style={styles.secondaryButtonFull} onClick={() => window.open(result.invoice_url, "_blank", "noopener,noreferrer")}>Open payment details</button>}
            </div>
          )}
        </>
      )}
